...
extension ViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        // Determine the actual button displayed
        var buttonToDisplay = calcButton
        if calcButton == .allClear || calcButton == .delete {
            let shouldShowDelete = !viewModel.model.isResultDisplayed && viewModel.calcHeaderLabel != "0"
            buttonToDisplay = shouldShowDelete ? .delete : .allClear
        ...
        // Call the appropriate method in the ViewModel
        viewModel.didSelectButton(with: buttonToDisplay)
        
        // Update the 'AC' or 'Del' button title based on the current state
        let headerHasContent = !viewModel.calcHeaderLabel.isEmpty && viewModel.calcHeaderLabel != "0"
        if let acButtonIndex = viewModel.calcButtonCells.firstIndex(where: { $0 == .allClear || $0 == .delete }) {
            if let acCell = collectionView.cellForItem(at: IndexPath(row: acButtonIndex, section: 0)) as? Views {
                let shouldShowDelete = !viewModel.model.isResultDisplayed && viewModel.calcHeaderLabel != "0"
                acCell.updateButtonTitleForClear(hasContent: shouldShowDelete)
            }
        }
    }
